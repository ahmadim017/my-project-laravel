...
<div class="col-md-8">
    <div class="card shadow mb-4">
      ...
      <form enctype="multipart/form-data"  class="bg-white shadow-sm p-3" action="{{route('datapenyedia.store')}}" method="POST">
          @csrf
        <label for="name">Nama Perusahaan</label>
        <input class="form-control {{$errors->first('namaperusahaan') ? "is-invalid" : ""}}" value="{{old('namaperusahaan')}}" placeholder="Nama Perusahaan" type="text" name="namaperusahaan"/>
        ...
        <select name="bentukusaha" class="form-control">
          ...
          <textarea name="alamat" class="form-control {{$errors->first('alamat') ? "is-invalid" : ""}}" value="{{old('alamat')}}" placeholder="Alamat Lengkap"></textarea>
          <div class="invalid-feedbeck">
            {{$errors->first('alamat')}}
          </div>
          <br>
